...
<div id="luckfox_model_mark" style={{display:'none'}}>3.5″ RPi LCD RTP</div>

## 1. 介绍

3.5″ RPi LCD RTP 是一款支持 SPI 接口的电阻触摸显示屏，配备 ST7796S 显示驱动和 XPT2046 触控芯片，具备 320×480 分辨率和 262K 色彩，显示清晰，触控灵敏。
...
| **逻辑电压** | 3.3V                              |
| **通信接口** | SPI   |
| **控制芯片** | Display：ST7796S <br/>Touch：XPT2046 |


## 3. 引脚定义 <br/>![](photo/3.5inch-RPi-LCD-RTP/3.5-RPi-LCD-RTP-details-13.jpg)

| **引脚号** | **标识**     | **描述**                                                 |
|------------|--------------|--------------------------|
| 1, 3, 5, 7, 8, 10, 16, 17, 18, 22         | NC           | 无连接    |
| 2, 4          | 5V           | 电源正极（5V）          |
...
| 21         | MISO         | SPI 主输入/从输出 |
| 23         | SCLK         | SPI 时钟引脚     |
| 24         | CE0          | LCD 片选信号（低电平有效） |
| 26       | CE1           | 触摸片选信号引脚 |

## 4. 产品尺寸<br/>![](photo/3.5inch-RPi-LCD-RTP/3.5-RPi-LCD-RTP-details-size.jpg)
## 5. 硬件连接
1.  将模块 26 针插座对齐连接至 Raspberry Pi GPIO 40 排针。
2.  推荐使用铜柱固定，避免松动。<br/>![](photo/3.5inch-RPi-LCD-RTP/3.5-RPi-LCD-RTP-details-15.jpg)
## 6.  软件设置
### 6.1. 开启SPI接口
打开终端，执行以下命令启用相关接口：
```
...
st7796s
```
如下图:<br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_st7796.png) <br/>
处理内核模块依赖：
```
sudo depmod -a
```
...
sudo nano /boot/firmware/config.txt
```
注释掉双屏相关配置。<br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_config_dtbo1.png)
...
1. 打开 **Screen Configuration** 应用程序  <br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_rotate_1.png)

2. 勾选 "Touchscreen"  <br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_rotate_2.png)<br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_rotate_3.png)

3. 进入Orientation，勾选需要旋转的角度。<br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_rotate_4.png)

### 命令行旋转
>💡 推荐在 Wayland 环境下使用此命令方式，适用于 Raspberry Pi OS Bookworm 及以上系统版本。
1. 安装 `wlr-randr` 工具
```
...
wlr-randr
```
输出示例（以 SPI-1 为例）：<br/>![](photo/3.5inch-RPi-LCD-CTP/3.5F_rotate_5.png)
3. 执行旋转命令
```
...
## 10. 资料
### 原理图
* [原理图](/PDF/3inch5-RPi-LCD-RTP/3.5inch-RPi-LCD-luckfox.pdf)
### 示例程序
* [示例程序](https://files.luckfox.com/wiki/Luckfox/Display/3inch5-RPi-LCD-RTP/3inch5_RPi_LCD_RTP_RPI_Demo.zip)
### 数据手册
* [ST7796S 数据手册](/PDF/3inch5-RPi-LCD-RTP/ST7796S_Datasheet.pdf)
* [XPT2046 数据手册](/PDF/3inch5-RPi-LCD-RTP/XPT2046_Datasheet.pdf)
### 图纸
* [3D图纸-STP](https://files.luckfox.com/wiki/Luckfox/Display/3inch5-RPi-LCD-RTP/3.5inch-RPI-LCD_RTP-20250523.stp)
* [2D图纸-DXF](https://files.luckfox.com/wiki/Luckfox/Display/3inch5-RPi-LCD-RTP/3.5inch-RPI-LCD_RTP-20250523.dxf)
* [2D图纸-PDF](/PDF/3inch5-RPi-LCD-RTP/3.5inch-RPI-LCD_RTP-20250523.pdf)